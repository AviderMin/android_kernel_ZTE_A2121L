...
#ifdef CONFIG_ZTE_USE_HEADPHONE_SWITCH
#define HPH_SW_ON	1
#define HPH_SW_OFF	0

/*GPIO on which hph switch control signal is coming.*/
int cdc_hph_sw_gpio = -1;
int cdc_hph_sw_state = HPH_SW_OFF;
struct mutex cdc_hph_sw_mutex;
#endif
...
#ifdef CONFIG_ZTE_USE_HEADPHONE_SWITCH
static char const *cdc_hph_sw_text[] = {"Off", "On"};

static const struct soc_enum cdc_hph_sw_ctl_enum =
		SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(cdc_hph_sw_text), cdc_hph_sw_text);



static int cdc_hph_sw_state_get(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: hph_sw_state = %d\n", __func__, cdc_hph_sw_state);
	ucontrol->value.enumerated.item[0] = cdc_hph_sw_state;
	return 0;
}

static int cdc_hph_sw_state_put(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_lock(&cdc_hph_sw_mutex);
	if (gpio_is_valid(cdc_hph_sw_gpio)) {
		gpio_set_value(cdc_hph_sw_gpio, state);
		cdc_hph_sw_state = state;
	} else {
		pr_err("%s: cdc_hph_sw_gpio %d is invalid\n",
			__func__, cdc_hph_sw_gpio);
	}
	mutex_unlock(&cdc_hph_sw_mutex);
...
#ifdef CONFIG_ZTE_USE_HEADPHONE_SWITCH
	SOC_ENUM_EXT("Headphone Switch", cdc_hph_sw_ctl_enum,
			cdc_hph_sw_state_get, cdc_hph_sw_state_put),
#endif
...
#ifdef CONFIG_ZTE_USE_HEADPHONE_SWITCH
	mutex_init(&cdc_hph_sw_mutex);
	cdc_hph_sw_gpio = of_get_named_gpio(pdev->dev.of_node,
				"zte,hph-sw-gpio", 0);
	dev_err(&pdev->dev, "%s: %s cdc_hph_sw_gpio %d\n",
			__func__, "zte,hph-sw-gpio", cdc_hph_sw_gpio);
	if (cdc_hph_sw_gpio < 0) {
		dev_err(&pdev->dev, "%s: %s property not found %d\n",
			__func__, "zte,hph-sw-gpio", cdc_hph_sw_gpio);
	} else {
		ret = gpio_request(cdc_hph_sw_gpio, "hph_sw_gpio");
		if (ret) {
			dev_err(&pdev->dev,
				"%s: cdc_hph_sw_gpio request failed, ret:%d\n",
				__func__, ret);
			if (cdc_hph_sw_gpio >= 0) {
				dev_dbg(&pdev->dev, "%s free cdc_hph_sw_gpio %d\n",
					__func__, cdc_hph_sw_gpio);
				gpio_free(cdc_hph_sw_gpio);
				cdc_hph_sw_gpio = -1;
			}
		} else {
			/*set the switch to off by default*/
			if (gpio_is_valid(cdc_hph_sw_gpio)) {
				gpio_direction_output(cdc_hph_sw_gpio, HPH_SW_OFF);
			}
		}
		cdc_hph_sw_state = HPH_SW_OFF;
	}
#endif
	return 0;
}
