...
/* ZTE_chenjun */
	...
	case FSA_SET_MIC_TO_SBU1:
		fsa4480_usbc_update_settings(fsa_priv, 0x7, 0x9F);
#if 0
		regmap_read(fsa_priv->regmap, 0x5, &reg_value);
		dev_info(fsa_priv->dev, "fsa4480_reg(%s)_SBU1: reg[0x5]=0x%x\n", __func__, reg_value);
#endif
		break;
	case FSA_SET_MIC_TO_SBU2:
		fsa4480_usbc_update_settings(fsa_priv, 0x0, 0x9F);
#if 0
		regmap_read(fsa_priv->regmap, 0x5, &reg_value);
		dev_info(fsa_priv->dev, "fsa4480_reg(%s)_SBU2: reg[0x5]=0x%x\n", __func__, reg_value);
#endif
		break;
