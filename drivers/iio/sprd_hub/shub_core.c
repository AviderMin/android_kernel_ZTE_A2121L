...
static short als_rgb_cali_coef = -1;
static int set_als_rgb_calib_cmd(struct shub_data *sensor)
{
	int err, i, status, light_sum = 0;
	u8 data[2];
	u16 *ptr = (u16 *)data;
	...
	for (i = 0; i < LIGHT_CALI_DATA_COUNT; i++) {
		...
		/*sleep for light senor collect data every 100ms*/
		msleep(100);
		pr_info("shub_sipc_read: als_data[%d] = %d\n", i, ptr[0]);
		light_sum += ptr[0];
	}

	als_rgb_cali_coef = light_sum / LIGHT_CALI_DATA_COUNT;
	pr_info("light sensor rgb cali light_sum:%d, als_rgb_cali_coef = %d\n",
		light_sum, als_rgb_cali_coef);

	status = CALIB_STATUS_PASS;
	return status;
}

static ssize_t light_sensor_rgb_calibrator_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct shub_data *sensor = dev_get_drvdata(dev);
	...
	if (sensor->cal_cmd != CALIB_DATA_WRITE ||
		sensor->cal_id != SENSOR_TYPE_LIGHT) {
		als_rgb_cali_coef = -1;
		pr_err("light sensor rgb cali cmd error\n");
		return -EINVAL;
	}

	err = set_als_rgb_calib_cmd(sensor);
	if (err !=  CALIB_STATUS_PASS) {
		als_rgb_cali_coef = -1;
		pr_err("light sensor rgb cali Fail!\n");
	}

	return err < 0 ? err : count;
}

static ssize_t light_sensor_rgb_calibrator_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 10, "%d", als_rgb_cali_coef);
}
static DEVICE_ATTR_RW(light_sensor_rgb_calibrator);
	...
	const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d", &als_mode) != 1)
		return -EINVAL;

	shub_send_command(sensor, SENSOR_LIGHT, SHUB_SET_MODE,
				&als_mode, sizeof(als_mode));

	light_als_mode = als_mode;

	return count;
}

static ssize_t als_mode_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
...
void set_als_mode(u8 mode)
{
	if (g_sensor == NULL)
		return;

	shub_send_command(g_sensor, SENSOR_LIGHT,
		SHUB_SET_MODE, &mode, sizeof(mode));
}
#endif
...
static struct attribute *sensorhub_attrs[] = {
	...
	&dev_attr_light_sensor_rgb_calibrator.attr,
