...
static int get_batt_coulomb_counting(struct charger_manager *cm, int *clb_cnt)
{
	union power_supply_propval val;
	struct power_supply *fuel_gauge;
	int ret;
...
static bool is_battery_charging(struct charger_manager *cm)
{
	int i, ret;
	bool charging = false;
	struct power_supply *psy;
...
/* 2 centigrade need report to user space */
...
static int cm_check_thermal_status(struct charger_manager *cm)
{
	...
	if (ret == CM_EVENT_BATT_OVERHEAT || ret == CM_EVENT_BATT_COLD)
		pr_info("%s: upper_limit=%d, lower_limit=%d, cm_event status=%d\n",
				__func__, upper_limit, lower_limit, ret);
	return ret;
}
				      ...
				      int jeita_status)
{
	...
	int term_volt, target_cur, ret = -ENODEV;
#else
	union power_supply_propval val;
	struct power_supply *psy;
...
#ifdef CONFIG_VENDOR_POWER_VOTER
	dev_info(cm->dev, "jeita use vote, ignore thermal current %d\n", cm->desc->thm_adjust_cur);
#else
...
#ifdef CONFIG_VENDOR_POWER_VOTER
	dev_info(cm->dev, "jeita use vote, ignore chg_limit_cur %d\n", chg_limit_cur);
#else
	if (target_cur > chg_limit_cur && chg_limit_cur != 0) {
		dev_info(cm->dev, "chg_limit_cur %d is less than target_cur %d\n",
			chg_limit_cur, target_cur);
	...
	dev_info(cm->dev, "jeita: target terminate voltage = %d, target current = %d\n",
		...
		dev_info(cm->dev, "%s jeita: vote: fcv %d, fcc %d\n", __func__, term_volt, target_cur);
#else
	for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
		psy = power_supply_get_by_name(cm->desc->psy_charger_stat[i]);
		if (!psy) {
		...
		if (ret) {
			power_supply_put(psy);
			dev_err(cm->dev,
				"failed to set terminate voltage to %s, ret = %d\n",
				cm->desc->psy_charger_stat[i], ret);
			continue;
		}
		...
		if (ret) {
			dev_err(cm->dev,
				"failed to set charge current to %s, ret = %d\n",
				cm->desc->psy_charger_stat[i], ret);
			continue;
		}
	}
...
static int cm_manager_jeita_current_monitor(struct charger_manager *cm)
{
	...
	if (desc->jeita_disabled) {
		if (last_jeita_status != STATUS_T2_TO_T3) {
			...
			cm->jeita_status = STATUS_T2_TO_T3;
	...
	cm->jeita_status = cur_jeita_status;

	if (cm->jeita_status == STATUS_BELOW_T0)
		cm->health = POWER_SUPPLY_HEALTH_COLD;
	else if (cm->jeita_status <= STATUS_T1_TO_T2)
		cm->health = POWER_SUPPLY_HEALTH_COOL;
	else if (cm->jeita_status == STATUS_ABOVE_T4)
		cm->health = POWER_SUPPLY_HEALTH_OVERHEAT;
	else if (cm->jeita_status == STATUS_T3_TO_T4)
...
static bool _cm_monitor(struct charger_manager *cm)
{
	int temp_alrt, ret;
	int i;
	int last_emergency_stop = 0, last_charging_status = 0;
#ifdef CONFIG_VENDOR_POWER_VOTER
	struct charger_policy *policy = &(cm->policy);
#endif
	...
	if (temp_alrt || cm->emergency_stop)
		dev_info(cm->dev, "temp_alrt %d, emergency_stop %d\n", temp_alrt, cm->emergency_stop);

	/* It has been stopped already */
	if (temp_alrt && cm->emergency_stop) {
		dev_warn(cm->dev,
			 "Emergency stopped already, temperature alert = %d\n", temp_alrt);
		return false;
	}
	...
	if (cm->parallel_charge_enabled && !cm->should_en_parallel_chg) {
		...
		if (cm->desc->psy_hardware2)
			charger_set_prop_by_name(cm->desc->psy_hardware2,
				POWER_SUPPLY_PROP_STATUS, false);
	...
	if (ret && ret != -EAGAIN) {
		dev_warn(cm->dev,
			 "Errors orrurs when adjusting charging current, ret=%d\n", ret);
		return false;
	}
	...
	if (temp_alrt) {
		...
		if (!is_battery_charging(cm)) {
			cm->emergency_stop = temp_alrt;
			power_supply_changed(cm->charger_psy);
			dev_info(cm->dev, "battery stop charging\n");
		}
	/*
	 ...
	 */
		...
		last_emergency_stop = cm->emergency_stop;
		last_charging_status = cm->charging_status;
		cm->emergency_stop = 0;
		cm->charging_status = 0;
		if (is_ext_pwr_online(cm)) {
			dev_info(cm->dev, "No emergency stop, try charging\n");
		...
		if (!is_battery_charging(cm)) {
			cm->emergency_stop = last_emergency_stop;
			cm->charging_status = last_charging_status;
			dev_info(cm->dev, "battery recover charging failed from emergency_stop %d\n",
					cm->emergency_stop);
		}
	}
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (cm->desc->cap_debug != -1)
			...
			cap_new  = cm->desc->cap;
			cap_new = DIV_ROUND_CLOSEST(cap_new * 101, 980);
	...
	case POWER_SUPPLY_PROP_CAPACITY_RAW:
		ret = get_batt_cap(cm, &val->intval);
		val->intval = ret ? cm->desc->cap : val->intval;

		val->intval = DIV_ROUND_CLOSEST(val->intval, 10);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_OCV:
		get_batt_ocv(cm, &val->intval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		for (i = 0; cm->desc->psy_charger_stat[i]; i++) {
			...
			power_supply_put(psy);
			if (ret) {
				dev_err(cm->dev, "get VOLTAGE_MAX failed\n");
				continue;
			}
		}
		break;
	case POWER_SUPPLY_PROP_CURRENT_COUNTER_ZTE:
		get_batt_coulomb_counting(cm, &val->intval);
		break;
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
		...
		power_supply_put(psy);
		if (ret)
			dev_err(cm->dev, "get ship mode failed\n");
		break;
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		val->intval = get_client_vote(policy->fcc_votable, POLICY_SETTING_VOTER);
		val->intval = (val->intval < 0) ? 1 : 0;
		break;
#endif
		     ...
		     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		...
		dev_info(cm->dev, "%s thermal control,vote: usb_icl %d, fcc %d, cur_jeita_status %d\n",
					__func__, val->intval, val->intval, cur_jeita_status);
#else

		dev_info(cm->dev, "%s thermal control, charge current limit, target val %d, actual val %d, cur_jeita_status %d\n",
					__func__, val->intval, thermal_val.intval, cur_jeita_status);
	...
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
		...
		power_supply_put(psy);
		if (ret)
			dev_err(cm->dev, "set ship mode failed\n");
		break;
#ifdef CONFIG_VENDOR_POWER_VOTER
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		...
		vote(policy->fcc_votable, POLICY_SETTING_VOTER, (bool)!val->intval, 0);
		if (val->intval == 0) {
			...
			if (cm->desc->psy_hardware2)
				charger_set_prop_by_name(cm->desc->psy_hardware2,
					POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, 0);
			#endif
			if (cm)
			...
			if (cm->desc->psy_hardware2)
				charger_set_prop_by_name(cm->desc->psy_hardware2,
					POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, val->intval);
			#endif
			if (cm)
			...
			unsigned long event, void *ptr)
{
	...
	if (cable->cm)
		cable->cm->emergency_stop = CM_EVENT_UNKNOWN;
	else
		return NOTIFY_BAD;
...
static struct charger_desc *of_cm_parse_desc(struct device *dev)
{
	...
	pr_info("poll_mode: %d, polling_interval_ms: %d, fullbatt_vchkdrop_ms: %d, fullbatt_vchkdrop_uV: %d, "
		...
		"battery_present: %d, fullbatt_warm_uV: %d\n",
		...
		desc->battery_present, desc->fullbatt_warm_uV);
	...
	of_property_read_string(np, "cm-hardware-psy", &desc->psy_hardware);
	...
	if (ret) {
		...
		pr_info("cm-hardware2-psy config!\n");
	...
	desc->disable_charger_type_jeita = true;
...
static int cm_get_bat_info(struct charger_manager *cm)
{
	...
	fuel_gauge = power_supply_get_by_name(cm->desc->psy_fuel_gauge);
	if (fuel_gauge) {
		ret = power_supply_get_property(fuel_gauge, POWER_SUPPLY_PROP_BATT_ID_INDEX, &prop);
		if (ret)
			pr_err("failed to battery id index, error:%d.\n", ret);
		else {
			batt_id_index = prop.intval;
			dev_info(cm->dev, "%s: battery id index: %d\n", __func__, prop.intval);
		}
	}
	/*
	 ...
	 */
	table = power_supply_find_ocv2cap_table(&info, 20, batt_id_index, &cm->desc->cap_table_len);
	if (!table)
		return -EINVAL;
...
static void cm_batt_works(struct work_struct *work)
{
	...
	case POWER_SUPPLY_STATUS_FULL:
		...
		} else if ((batt_ocV < (cm->desc->fullbatt_uV - cm->desc->fullbatt_vchkdrop_uV - 50000))
		    && (bat_uA < 0)) {
			cm->desc->force_set_full = false;
...
static int charger_set_prop_by_name(const char *name, enum power_supply_property psp, int data)
{
	...
	if (name == NULL) {
		vote_debug("psy name is NULL!!\n");
		goto failed_loop;
	}

	psy = power_supply_get_by_name(name);
	if (!psy) {
		vote_debug("get %s psy failed!!\n", name);
		goto failed_loop;
	}
	...
	rc = power_supply_set_property(psy,
				psp, &val);
	if (rc < 0) {
		vote_debug("Failed to set %s property:%d rc=%d\n", name, psp, rc);
		power_supply_put(psy);
		return rc;
	}
...
static int charger_get_prop_by_name(const char *name, enum power_supply_property psp, int *data)
{
	...
	rc = power_supply_get_property(psy,
				psp, &val);
	if (rc < 0) {
		vote_debug("Failed to get %s property:%d rc=%d\n", name, psp, rc);
		power_supply_put(psy);
		return rc;
	}
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		pval->intval = get_client_vote(policy->fcc_votable, POLICY_SETTING_VOTER);
		pval->intval = (pval->intval < 0) ? 1 : 0;
		break;
	default:
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		...
		vote(policy->fcc_votable, POLICY_SETTING_VOTER, (bool)!pval->intval, 0);
		break;
	default:
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (pval->intval) {
			...
			rerun_election(policy->usb_icl_votable);
			rerun_election(policy->topoff_votable);
			rerun_election(policy->recharge_voltage_votable);
		...
		charger_set_prop_by_name(cm->desc->psy_hardware,
			POWER_SUPPLY_PROP_STATUS, pval->intval);
...
static int charger_manager_policy_init_battery_voter(struct charger_manager *cm)
{
	...
	ret = power_supply_get_battery_info(policy->hook_psy, &bat_info);
	if (ret) {
...
static void policy_usb_change_handler_work(struct work_struct *work)
{
	...
	if (info->limit) {
		...
		vote(info->usb_icl_votable, CHARGER_TYPE_VOTER, false, 0);
...
static int charger_manager_policy_init(struct charger_manager *cm)
{
	...
	if (IS_ERR(cm->policy.usb_phy)) {
		vote_debug("######@failed to find USB phy %d\n", (int)PTR_ERR(cm->policy.usb_phy));
	...
	if (ret) {
		vote_debug("failed to register notifier:%d\n", ret);
		return ret;
	}
	...
	list_for_each_entry(cm, &cm_list, entry) {
		if (match_string(cm->desc->psy_charger_stat, -1,
				 ...
				 match_string(&cm->desc->psy_fuel_gauge,
					      -1, psy->desc->name) >= 0 ||
					      match_string(&cm->desc->psy_hardware,
					      -1, psy->desc->name) >= 0) {
