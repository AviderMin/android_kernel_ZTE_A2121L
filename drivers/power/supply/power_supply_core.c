				  ...
				  struct power_supply_battery_info *info)
{
	...
	info->charge_full_design2_uah        = -EINVAL;
	...
	info->factory_internal_resistance2_uohm  = -EINVAL;
	...
	for (index = 0; index < POWER_SUPPLY_OCV_TEMP_MAX; index++) {
		...
		info->ocv_table2[index]       = NULL;
		info->ocv_temp[index]        = -EINVAL;
		info->ocv_table_size[index]  = -EINVAL;
		info->ocv_table2_size[index]  = -EINVAL;
	}
	...
	of_property_read_u32(battery_np, "charge-full-design2-microamp-hours",
			     &info->charge_full_design2_uah);
	...
	of_property_read_u32(battery_np, "factory-internal-resistance2-micro-ohms",
			     &info->factory_internal_resistance2_uohm);
	...
	for (index = 0; index < len; index++) {
		...
		propname = kasprintf(GFP_KERNEL, "ocv-capacity-table2-%d", index);
		list = of_get_property(battery_np, propname, &size);
		if (!list || !size) {
			dev_err(&psy->dev, "failed to get %s\n", propname);
			kfree(propname);
		...
		kfree(propname);
		tab_len = size / (2 * sizeof(__be32));
		info->ocv_table2_size[index] = tab_len;

		table = info->ocv_table2[index] =
			devm_kzalloc(&psy->dev, tab_len * sizeof(*table),
				     GFP_KERNEL);
		if (!info->ocv_table2[index]) {
			dev_info(&psy->dev, "kzalloc failed for ocv_table2 index: %d\n", index);
			for (i = 0; i < index; i++) {
				kfree(info->ocv_table2[i]);
				info->ocv_table2[i] = NULL;
			}
			break;
		}
		...
		for (i = 0; i < tab_len; i++) {
			table[i].ocv = be32_to_cpu(*list++);
			table[i].capacity = be32_to_cpu(*list++);
			dev_dbg(&psy->dev, "#2 index %d: [%d]: ocv %d, cap: %d\n",
						index, i, table[i].ocv, table[i].capacity);
		}
	}
				   ...
				   struct power_supply_battery_info *info)
{
	...
	for (i = 0; i < POWER_SUPPLY_OCV_TEMP_MAX; i++) {
		...
		if (info->ocv_table2[i])
			devm_kfree(&psy->dev, info->ocv_table2[i]);
	}
...
power_supply_find_ocv2cap_table(struct power_supply_battery_info *info,
				int temp, int batt_id_index, int *table_len)
{
	...
	if (info->ocv_table2[0] && batt_id_index == 1) {
		*table_len = info->ocv_table2_size[best_index];
		return info->ocv_table2[best_index];
	}
...
int power_supply_batinfo_ocv2cap(struct power_supply_battery_info *info,
				 int ocv, int temp, int batt_id_index)
{
	struct power_supply_battery_ocv_table *table;
	int table_len;

	table = power_supply_find_ocv2cap_table(info, temp, batt_id_index, &table_len);
	if (!table)
		return -EINVAL;
