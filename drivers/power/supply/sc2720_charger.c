...
#define SC2720_CHG_CURRENT_MAX			1300
...
#define SC2720_CHG_TERM_CURRENT_RATIO_10	3
#define SC2720_CHG_TERM_CURRENT_RATIO_20	2
#define SC2720_CHG_TERM_CURRENT_RATIO_40	1
#define SC2720_CHG_TERM_CURRENT_RATIO_90	0
...
#define SC2720_CHG_TERMINATION_CURRENT_MASK	GENMASK(2, 1)
...
static int sc2720_set_termination_current(struct sc2720_charger_info *info, u32 term_cur)
{
	int val, ret;
	int ratio = 0;
	...
	if (ratio <= CHG_TERM_CURRENT_RATIO_10)
		val = SC2720_CHG_TERM_CURRENT_RATIO_10;
	else if (ratio <= CHG_TERM_CURRENT_RATIO_20)
		val = SC2720_CHG_TERM_CURRENT_RATIO_20;
	else if (ratio <= CHG_TERM_CURRENT_RATIO_40)
		val = SC2720_CHG_TERM_CURRENT_RATIO_40;
	else
		val = SC2720_CHG_TERM_CURRENT_RATIO_90;
	...
	ret = regmap_update_bits(info->regmap,
				 info->base + SC2720_CHG_CFG0,
				 SC2720_CHG_TERMINATION_CURRENT_MASK,
				 val << SC2720_CHG_TERMINATION_CURRENT_MASK_SHIT);
	if (ret) {
...
static int sc2720_get_termination_current(struct sc2720_charger_info *info,
				      u32 *term_cur)
{
	int ret;
	u32 val, term_cur_set;
	int chg_cur;
	int ratio;
	...
	ret = sc2720_charger_get_current(info, &chg_cur);
	if (ret) {
		dev_err(info->dev, "failed to read charge current\n");
	}

	term_cur_set = (val & SC2720_CHG_TERMINATION_CURRENT_MASK) >> SC2720_CHG_TERMINATION_CURRENT_MASK_SHIT;
	if (term_cur_set == SC2720_CHG_TERM_CURRENT_RATIO_10)
		ratio = CHG_TERM_CURRENT_RATIO_10;
	else if (term_cur_set == SC2720_CHG_TERM_CURRENT_RATIO_20)
		ratio = CHG_TERM_CURRENT_RATIO_20;
	else if (term_cur_set == SC2720_CHG_TERM_CURRENT_RATIO_40)
...
static void sc2720_charger_dump_regs(struct sc2720_charger_info *info)
{
	u32 val_ctr0, val_ctr1, val_status;

	regmap_read(info->regmap, info->base + SC2720_CHG_CFG0, &val_ctr0);
	regmap_read(info->regmap, info->base + SC2720_CHG_CFG1, &val_ctr1);
	regmap_read(info->regmap, info->base + SC2720_CHG_STATUS, &val_status);

	dev_info(info->dev, "info->charging: %d, info->limit: %d\n",
				info->charging, info->limit);
...
static int sc2720_charger_set_current(struct sc2720_charger_info *info, u32 cur)
{
	...
	if (cur > SC2720_CHG_CURRENT_MAX)
		cur = SC2720_CHG_CURRENT_MAX;
					   ...
					   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		ret = sc2720_charger_get_current(info, &val->intval);
		if (ret)
				goto out;
		break;

	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		ret = sc2720_get_termination_current(info, &val->intval);
		if (ret) {
	...
	default:
		pr_info("%s error: default case: %d\n", __func__, psp);
		ret = -EINVAL;
	}
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		...
		if (val->intval == 0) {
			ret = sc2720_charger_set_status(info, 0);
			if (ret < 0)
				dev_err(info->dev, "set charge status failed\n");
			dev_info(info->dev, "set charge current %d, stop charge\n", val->intval);
			break;
		}
		ret = sc2720_charger_set_status(info, 1);
		if (ret < 0)
			dev_err(info->dev, "set charge status failed\n");
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		/* fall through */
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
		dev_info(info->dev, "set fcv psp %d, val = %d\n", psp, val->intval);
	...
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		ret = sc2720_set_termination_current(info, val->intval);
		if (ret)
			dev_err(info->dev, "failed to set charge termination current\n");
