...
#define SC27XX_FGU_BOOT_CAPACITY_THRESHOLD	1001
...
#define SC27XX_FGU_POCV_VOLT_THRESHOLD_MV	3400
#define SC27XX_FGU_CAP_GAP_THRESHOLD	50
...
static int sc27xx_fgu_get_ocv_cap(struct sc27xx_fgu_data *data, int *cap)
{
	int ret = -1;
	int ocv = -1;
	...
	if (ret) {
		dev_err(data->dev, "Failed to read vbat ocv, ret = %d\n", ret);
		return -EINVAL;
	}

	*cap = power_supply_ocv2cap_simple(data->cap_table, data->table_len, ocv);
	SPRD_FGU_DEBUG("sc27xx_fgu_get_ocv_cap cap=%d\n", *cap);
	return 0;
}
...
static int sc27xx_fgu_get_boot_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	...
	int ocv_now = 3800000, cap_ocv_now = 500, cur_now = 0, saved_cap = -1;
	int temp_cap = 100, delta_cap_normal = 0, delta_cap_cold = 0;
	...
	if (ocv <= 0) {
		SPRD_FGU_DEBUG("%s: POCV %dmv is too low, use vbat_ocv instead\n", __func__, ocv);
		ocv = ocv_now;
	}
	data->boot_vol = ocv;
	...
	*cap = power_supply_ocv2cap_simple(data->cap_table, data->table_len,
					   ocv);
	cap_ocv_now = power_supply_ocv2cap_simple(data->cap_table, data->table_len,
					   ocv_now) * FULL_RAW_CAP_FACTOR;

	if (data->cap_table_len > 0) {
		temp_cap = sc27xx_fgu_temp_to_cap(data->cap_temp_table,
						  data->cap_table_len,
						  data->bat_temp);
		delta_cap_cold = (100 - temp_cap) * FULL_RAW_CAP_FACTOR;
		SPRD_FGU_DEBUG("boot thermal delta_cap_cold: %d\n", delta_cap_cold);
	}
	...
	*cap *= 10;
	...
	if (!is_first_poweron) {
		...
		if (ret) {
			dev_err(data->dev, "failed to read last cap\n");
			SPRD_FGU_DEBUG("#first_poweron: %d. final cap: %d, saved cap: %d, vbat: %d, bat_temp: %d, "
				"ocv: %d, oci: %d, cur_now: %d, ocv_now: %d, cap_ocv_now: %d rint: %d\n",
				is_first_poweron, *cap, saved_cap, volt, data->bat_temp, ocv, oci, cur_now,
				ocv_now, cap_ocv_now, data->internal_resist);
			return ret;
		}

		SPRD_FGU_DEBUG("not first_poweron! ocv cap: %d, saved_cap: %d\n", *cap, saved_cap);

		if (ocv_now <= SC27XX_FGU_OCV_CAPACITY_ZERO) {
			SPRD_FGU_DEBUG("ocv too low,  recaculate cap at boot: %d\n", cap_ocv_now);
			*cap = cap_ocv_now;
		} else if ((abs(*cap - saved_cap) < SC27XX_FGU_BOOT_CAPACITY_THRESHOLD) ||
		    (ocv >= SC27XX_FGU_DENSE_CAPACITY_LOW &&
		     ocv <= SC27XX_FGU_DENSE_CAPACITY_HIGH))
			*cap = saved_cap;
		data->boot_cap = *cap;

		if (*cap < delta_cap_cold) {
			data->boot_cap_cold_gap = delta_cap_cold - *cap;
			SPRD_FGU_DEBUG("boot capacity %d is less than thermal delta_cap_cold: %d\n",
				*cap, delta_cap_cold);
		}

		delta_cap_normal = *cap - cap_ocv_now;
		if (delta_cap_normal >= SC27XX_FGU_CAP_GAP_THRESHOLD) {
			data->boot_cap_save = *cap;
			data->boot_cap_gap = delta_cap_normal;
			SPRD_FGU_DEBUG("boot capacity %d is much more than ocv cap: %d\n", *cap, cap_ocv_now);
		}
	}
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATT_ID_INDEX:
				   ...
				   const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CALIBRATE:
		if (val->intval == 100 * FULL_RAW_CAP_FACTOR || val->intval == 0) {
			data->boot_cap_gap = -1;
			data->boot_cap_cold_gap = -1;
			data->boot_cap_save = -1;
		}
...
static enum power_supply_property sc27xx_fgu_props[] = {
	...
	POWER_SUPPLY_PROP_BATT_ID_INDEX,
			      ...
			      const struct sc27xx_fgu_variant_data *pdata)
{
	...
	if (data->batt_id_index == 1) {
		if (info.charge_full_design2_uah > 0)
			data->total_cap = info.charge_full_design2_uah / 1000;
		if (info.factory_internal_resistance2_uohm > 0)
			data->internal_resist = info.factory_internal_resistance2_uohm / 1000;
	}
	 ...
	 */
	table = power_supply_find_ocv2cap_table(&info, 20, data->batt_id_index, &data->table_len);
	if (!table)
		return -EINVAL;
