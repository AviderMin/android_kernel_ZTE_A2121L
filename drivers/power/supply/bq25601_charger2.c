				   ...
				   u32 limit_cur)
{
	u8 reg_val;
	int ret;
	bool en_highz = REG00_HIZ_DISABLE;

#ifdef ZTE_FEATURE_PV_AR
	if (limit_cur == 0) {
		en_highz = REG00_HIZ_ENABLE;
	...
	reg_val |= (en_highz << REG00_ENHIZ_SHIFT);

	ret = bq25601_write(info, BQ2560X_REG_00, reg_val);
					  ...
					  u32 val)
{
	...
	pr_info("bq25601_#2: set shipmode #####\n");
					...
					u32 vol_mv)
{
	...
	dev_info(info->dev, "set bq25601_#2 vindpm reg_val 0x%x\n", reg_val);
...
static int bq25601_charger_get_status(struct bq25601_charger_info *info)
{
	...
	pr_info("bq25601_#2: read status failed, ret = %d\n", ret);
...
static int bq25601_charger_dumper_reg(struct bq25601_charger_info *info)
{
	...
	pr_info("bq25601_#2: charging[%d], fcv[%d], usb_icl[%d], fcc[%d], topoff[%d], rechg_volt[%d]",
				...
				const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		pr_info("bq25601_#2[REG]: set fcc %d", val->intval);
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_RECHARGE_SOC:
		pr_info("bq25601_#2[REG]: set recharge_voltage %d", val->intval);
		...
		break;
	case POWER_SUPPLY_PROP_FEED_WATCHDOG:
		pr_info("bq25601_#2[REG]: feed watchdog\n");
	...
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
		if (val->intval == 0) {
			pr_info("bq25601_#2[REG]: set shipmode %d", val->intval);
			ret = bq25601_charger_set_shipmode(info, val->intval);
			if (ret < 0)
				dev_err(info->dev, "set shipmode failed\n");
		} else
			pr_info("bq25601_#2[REG]: set shipmode invalid val %d!", val->intval);
		break;
	case POWER_SUPPLY_PROP_TUNING_VINDPM:
		pr_info("bq25601_#2[REG]: tuning vindpm %d", val->intval);
...
static int bq25601_charger_hw_init(struct bq25601_charger_info *info)
{
	...
	if (ret) {
		...
		ret = bq25601_charger_set_limit_current(info, 0);
		if (ret)
			dev_err(info->dev, "set bq25601 set limit current failed\n");

		ret = bq25601_charger_set_current(info, 0);
		if (ret)
		...
		const struct i2c_device_id *id)
{
	...
	if (IS_ERR(info->gpiod)) {
		dev_warn(dev, "chg2 failed to get charger detection GPIO\n");
		info->gpiod = NULL;
	}
...
MODULE_DESCRIPTION("BQ25601 Charger2 Driver");
