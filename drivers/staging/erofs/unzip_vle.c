				     ...
				     bool reserve_allocation)
{
	...
	for (i = 0; i < clusterblks; ++i) {
		...
		page = find_get_page(mapping, start + i);
		found = find_get_page(mapping, start + i);
		if (!found) {
			noio = false;
...
repeat:
		...
		oldpage = READ_ONCE(compressed_pages[i]);
		page = READ_ONCE(compressed_pages[i]);
	...
	struct page **mpage_ret, int flags)
{
	...
	if (!mpage || mpage->index != e_blkaddr) {
		if (mpage)
			put_page(mpage);
