...
int sprd_panel_unprepare(struct drm_panel *p)
{
...
#endif
	{
		...
		DRM_INFO("disable lcd power regulator\n");
		mdelay(5);
...
int sprd_panel_prepare(struct drm_panel *p)
{
...
#endif
	{
		...
		if (ret < 0)
			DRM_ERROR("enable lcd power regulator failed\n");
		else
			DRM_INFO("enable lcd power regulator successfully\n");
		mdelay(5);
	}
#ifdef CONFIG_ZTE_LCDBL_I2C_CTRL_VSP_VSN
...
static void zte_set_lcd_ic_pwm(struct sprd_panel *panel)
{
	...
	if (g_zte_ctrl_pdata->zte_lcd_ctrl->backlight_ctrl_by_lcdic_pwm) {
		...
		if (g_zte_ctrl_pdata->zte_lcd_ctrl->lcd_bl_register_len > 8 &&
...
extern void set_als_mode(u8 als_mode);
#define ALS_BACKLIGHT_12bit 16
#define ALS_BACKLIGHT_10bit 8
#define ALS_HIGH_THD  100
#define ALS_LOW_THD   10
...
#endif
{
	...
	bl_preset = level;
		...
		if (g_zte_ctrl_pdata->zte_lcd_ctrl->lcd_bl_register_len == 12) {
			...
			if ((ALS_MODE == ALS_LOW) && (level > ALS_HIGH_THD*ALS_BACKLIGHT_12bit)) {
				...
				level = g_zte_ctrl_pdata->zte_lcd_ctrl->zte_convert_brightness(level, 2047);
#ifdef CONFIG_ZTE_LCD_SET_ALS_MODE
				if ((ALS_MODE == ALS_LOW) && (level > ALS_HIGH_THD*ALS_BACKLIGHT_10bit)) {
					set_als_mode(ALS_HIGH);
					ALS_MODE = ALS_HIGH;
				} else if ((ALS_MODE == ALS_HIGH) && (level < ALS_LOW_THD*ALS_BACKLIGHT_10bit)) {
					set_als_mode(ALS_LOW);
					ALS_MODE = ALS_LOW;
				}
#endif
	...
	#else
	if (likely(last_backlevel != level)) {
		ZTE_LCD_INFO("%s:brightness=%d level=%d --> convert_level = %d\n",
						__func__, brightness, bl_preset, level);
	} else {
		ZTE_LCD_INFO("%s:last_backlevel=%d is no set\n", __func__, last_backlevel);
		mutex_unlock(&panel_lock);
		return 0;
	}
	#endif
