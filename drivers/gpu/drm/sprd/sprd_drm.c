...
static void sprd_drm_shutdown(struct platform_device *pdev)
{
	...
	if (!drm) {
		...
		if (cali_mode == true) {
			DRM_WARN("zte add for cali_mode sprd_panel_unprepare\n");
...
static int is_pm_enter_resume = false;
#endif
static int sprd_drm_pm_suspend(struct device *dev)
{
	...
	if (g_zte_ctrl_pdata != NULL) {
		if ((g_zte_ctrl_pdata->last_dpms == DRM_MODE_DPMS_SUSPEND) ||
			(g_zte_ctrl_pdata->last_dpms == DRM_MODE_DPMS_STANDBY)) {
			ZTE_LCD_INFO("%s: panel in doze mode. skip last_dpms=%d\n",
...
#ifdef CONFIG_ZTE_LCD_HBM_CTRL
	if (g_zte_ctrl_pdata != NULL) {
		if ((is_pm_enter_resume) &&
			((g_zte_ctrl_pdata->last_dpms == DRM_MODE_DPMS_SUSPEND) ||
				(g_zte_ctrl_pdata->last_dpms == DRM_MODE_DPMS_STANDBY))) {
			ZTE_LCD_INFO("%s: panel in doze mode. skip last_dpms=%d\n",
						__func__, g_zte_ctrl_pdata->last_dpms);
			return 0;
