...
static void __iomem *clk_gate_base;

static struct clk *val_to_dpuclk(struct dpu_clk_context *ctx, u32 val)
{
	switch (val) {
	case 153600000:
...
static struct clk *val_to_dpiclk(struct dpu_clk_context *ctx, u32 val)
{
	if (val <= 153600000) {
		switch (val) {
		case 96000000:
				...
				struct device_node *np)
{
	...
	clk_gate_base = ioremap_nocache(0x323a0000, 0x10000);
...
static int dpu_clk_enable(struct dpu_context *ctx)
{
	...
	writel_relaxed(0x0, clk_gate_base + 0x0034);
