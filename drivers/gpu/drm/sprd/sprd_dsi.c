...
static void sprd_dsi_encoder_enable(struct drm_encoder *encoder)
{
	...
	/*
	* FIXME:
	* When last dpms is doze_suspend, panel remain on in a low power state and continue displaying
...
static void sprd_dsi_encoder_disable(struct drm_encoder *encoder)
{
	...
	if ((dsi->dpms == DRM_MODE_DPMS_STANDBY) &&
		...
		mutex_unlock(&dsi_lock);
		return;
	} else if ((dsi->dpms == DRM_MODE_DPMS_SUSPEND) &&
			 (dsi->last_dpms != DRM_MODE_DPMS_STANDBY)) {
		DRM_INFO("%s(invalid mode change)\n", __func__);
		mutex_unlock(&dsi_lock);
		return;
	}
			   ...
			   struct mipi_dsi_device *slave)
{
	...
	if (mipi_clk_from_uboot > 1000) {
		val = mipi_clk_from_uboot;
	}
	pr_info("[SPRD_LCD] phy-bit-clock is set to uboot value=%d", val);
	#else
	pr_info("[SPRD_LCD] phy-bit-clock is read from dtsi value=%d", val);
	#endif
	if (!ret) {
...
void sprd_dsi_lp_cmd_enable_func(struct drm_encoder *encoder,
		bool enable)
{
	struct sprd_dsi *dsi = encoder_to_dsi(encoder);
